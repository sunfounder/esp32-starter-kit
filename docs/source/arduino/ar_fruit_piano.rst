...
To use the touch sensor on the ESP32, you can use the ``touchRead()`` function 
in your code to read the touch sensor values on the designated touch pins. 
The ``touchRead()`` function returns a value between 0 and 4095, with a lower value indicating a stronger touch input.
...
.. note::
    ...
    * You can open the file ``6.1_fruit_piano.ino`` under the path of ``esp32-ultimate-kit\c\codes\6.1_fruit_piano`` directly.
