...
ESP32は、デュアルコア処理、Wi-FiおよびBluetoothを内蔵し、豊富な周辺インターフェースを備えた高性能で汎用性の高いマイクロコントローラです。低消費電力設計により、コンパクトで高性能なIoTアプリケーションに最適です。
...
* **多機能性**：シンプルな家電から複雑な産業機械まで、ESP32ボードは一貫した効率的な性能を提供します。

要するに、ESP32ボードは、堅牢な処理能力と多様な接続オプションを提供するだけでなく、IoTやスマートデバイス分野での選択肢としての特長を誇っています。
...
ESP32ボードのカメラとSDカード機能を最大限に活用できる拡張ボードを設計しました。OV2640カメラ、Micro SD、およびESP32ボードを組み合わせることで、オールインワンの拡張ボードが得られます。

このボードは、2種類のGPIOヘッダーを提供しています。一つは、素早いプロトタイピングプロジェクトに最適なメスヘッダー付き。もう一つはネジ端子を備えており、安定したワイヤ接続が可能で、IoTプロジェクトに適しています。

さらに、プロジェクトはPower Packで電源を供給することができます。Power Packの残量が少なくなった場合は、5VのUSBケーブルを接続するだけで充電することができます。これにより、屋外プロジェクトやリモートアプリケーションに最適なツールとなります。
...
* **電源スイッチ**
    * Power Packの電源供給を制御し、オン/オフを切り替えます。

* **充電ポート**
    * 5Vの USBケーブルを接続すると、Power Packを充電できます。

* **Power Packポート**
    * PH2.0-2Pインターフェースを採用し、Power Packと互換性があります。
    * ESP32ボードとESP32カメラ拡張の両方に電源を供給します。

* **ESP32ピンヘッダー**
    * ESP32ボードモジュール専用。向きに注意し、USBポートが同じ側を向いていることを確認して、誤った配置を避けてください。
...
* **インジケーターライト**
    * **PWR**: Power Packが電源供給されているか、USBがESP32に直接接続されているときに点灯します。
    * **CHG**: USBをボードの充電ポートに接続すると点灯し、充電が開始されたことを示します。Power Packが完全に充電されると、消灯します。
...
ESP32ボードのピン配置図は、 :ref:`esp32_pinout` にあります。

しかし、ESP32ボードを拡張ボードに挿入すると、そのピンの一部はMicro SDカードやカメラを駆動するためにも使用されることがあります。
...
**コードのアップロード**
    ...
    ESP32ボードにコードをアップロードする必要がある場合、USBケーブルを使用してコンピュータに接続します。
...
..     Power PackケーブルをPower Packポートに注意深く挿入し、バッテリーターミナルを上に押し上げるのを防ぐために過度な力を避けます。ターミナルが上に押し上げられた場合、ピンが壊れていなければ問題ありません。位置に戻すだけで良いです。

..     .. image:: ../img/plugin_battery.png
..         :width: 600
..         :align: center

..     Power Packが消耗している場合、5VのUSBを接続して充電します。
