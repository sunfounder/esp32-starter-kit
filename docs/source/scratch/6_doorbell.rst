...
* Connect one of the pins on the left side of the button to pin25, which is connected to a pull-down resistor and a 0.1uF (104) capacitor (to eliminate jitter and output a stable level when the button is working).
...
Use [if then] to make a conditional statement that when the value of the pin25 read is equal to 1 (the key is pressed), the sound **xylo1** will be played.
